...
export class SunData {
     ...
     * @returns SunJson - sun rise/set,  moon illuminaiton, phase, etc.
     */
    public async getSunData(lat: string, lon: string, apiKey: string, timeZone: string, dateStr = ""): Promise<SunJson | null> { 
        let sunJson: SunJson | null = null;
        ...
        try {
            ...
            sunJson = this.cache.get(key) as SunJson;
            if (sunJson !== null) {
                return sunJson;
            }
            ...
            await axios.get(url, options)
                .then((res: AxiosResponse) => {
                    ...
                    sunJson = res.data as SunJson;
                    sunJson.lunarAgeDays      = this.getMoonAgeDays(date);
                    sunJson.lunarIllumination = this.getMoonIllumination(sunJson.lunarAgeDays);
                    sunJson.lunarWaxWane      = this.getWaxWane(sunJson.lunarAgeDays);
                    sunJson.lunarPhase        = this.getPhaseStr(sunJson.lunarAgeDays);
                })
                .catch((error) => {
                    this.logger.warn(`SunData: No data: ${error})`);
                });
            ...
            this.cache.set(key, sunJson, midnightTonight.valueOf());            
        } catch (e) {
            if (e instanceof Error) {
                this.logger.error(`SunData: ${e.stack}`);
            } else {
                this.logger.error(`${e}`);
            }
            sunJson = null;
